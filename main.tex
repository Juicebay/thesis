...
\begin{tabular}{cccc}
\toprule
$p_{M_1}$ & $p_{M_2}$ & $p_{M_3}$ & $p_{m_1}$ \\
\midrule
\boxres{$c_1$} & \underres{$c_1$} & \boxres{$c_3$} & \underres{$c_2$} \\
$c_2$ & \boxres{$c_2$} & $c_2$ & \boxres{$c_1$} \\
\hline
$c_3$ & $c_3$ & $c_1$ & $c_3$ \\
\bottomrule
\end{tabular}
...
Under the \textbf{Original Policy} ($q^{M}_{c_1}=2$), the mechanism yields the matching $\{M_1:c_1, M_2:c_1, M_3:c_3, m_1:c_2\}$. The minority student $m_1$ receives their top choice, $c_2$.

Under the \textbf{Stronger Policy} ($\bar{q}^{M}_{c_1}=1$), the mechanism yields the matching $\{M_1:c_1, M_2:c_2, M_3:c_3, m_1:c_1\}$. Here, $m_1$ receives $c_1$ (their second choice). Since $c_2 p_{m_1} c_1$, the minority student is strictly worse off under the stronger affirmative action policy.
\end{proof}
...
The failure of minimal responsiveness in the Shanghai Mechanism contrasts with findings that
IA with reserves (IA-R) is minimally responsive, assuming truthful reporting \parencite{afacan2016affirmative}. However, it aligns with the general impossibility results for DA-based mechanisms
identified by \textcite{kojima2012school}. The Shanghai mechanism inherits the interconnected rejection chains of DA within each round, which drives this perverse outcome.
...
This issue is similar to the IA mechanism with reserves (IA-R). IA-R also fails to respect the
AAP because immediate (permanent) acceptance allows majority students to secure seats ahead of
minority students who apply later. In contrast, DA with reserves (DA-R) respects the AAP because
assignments remain tentative throughout the process \parencite{chaudhury2025affirmative}.
...
The failure of AA fairness in the Shanghai Mechanism highlights an issue related to rigidity of finialized assignments per round. The mechanism allows a lower-priority majority student to secure a seat in an early round. Once this assignment is finalized, the seat is permanently unavailable, even if a higher-priority student applies in a later round. This results in an unnecessary priority violation that is not justified by the affirmative action policy.
